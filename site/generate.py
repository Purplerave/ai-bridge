...
(function () {
  ...
  function renderTimeline(items){
    ...
    var html = '', last = null;
    items.slice(-14).reverse().forEach(function(it){
      if (it.thread !== last){
        if (last !== null) html += '<div class="thread"></div>';
    ...
    box.innerHTML = html;
    if (items.length > 14){
      box.insertAdjacentHTML('beforeend','<div class="hint">… y ' + (items.length-14) + ' mensajes más en el índice</div>');
    }
  }
  ...
  function boot(){
    ...
    fetchT(REPO + 'INDEX.md').then(function(text){
      ...
      document.getElementById('err-index').style.display = 'block';
      document.getElementById('err-index').textContent = 'No se pudo leer INDEX.md (' + e.message + '). Si estás en una vista previa sin red, ábrelo en GitHub Pages: https://purplerave.github.io/ai-bridge/';
